...
async function deleteResource(id) {
  const response = await fetch(URL_VIDEOS+id, {
    method: 'DELETE',
    headers: {
...
export default function ItemVideoSideBar({ video, actualizar=()=>{} }) {
  ...
  const eliminar = async () => {
    ...
    const resp = await deleteResource(video.id);
    actualizar()
  }
  ...
  return (
    <div className='flex items-center justify-center relative cursor-pointer group'>
        ...
        {showDropDown &&
          <div id="dropdown" className="text-start absolute top-8 left-0 z-10 bg-white divide-y divide-gray-100 dark:divide-gray-600 rounded-lg shadow w-44 dark:bg-gray-700">
            <div className='px-2 py-2 text-sm'>
              <h6 className='truncate'>{video.nombre}</h6>
            </div>  
            <ul className="pb-2 py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
              <li>
                <button onClick={()=>{descargarImagen(video.video, video.nombre); setShowDropDown(!showDropDown)}} className="flex gap-2 items-center px-3 py-2 text-start w-full hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                  <FaDownload />
                  Descargar
                </button>
              </li>
      ...
      <Image
        className=''
        src={video.miniatura.imagen}
        width={video.miniatura.width}
        alt=''
        height={video.miniatura.height}
        blurDataURL={video.miniatura.blurBase64}
        placeholder={video.miniatura.blurBase64 ? 'blur' : 'empty'}
      />
      }
